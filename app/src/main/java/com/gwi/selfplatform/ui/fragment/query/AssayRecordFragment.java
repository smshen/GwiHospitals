...
import com.gwi.selfplatform.ui.activity.query.AssayQueryDetailCommActivity;
...
/**
 * 化验查询内容页面(依据TAG的不同，做不同的处理)
 *
 * @version v2.0
 * @date 2015-12-15
 */
public class AssayRecordFragment extends AbstractRecordFragment {
    private static final String TAG = AssayRecordFragment.class.getSimpleName();
    private static final int REQUEST_CODE_BIND = 0x007;

    private List<G2110> mDataList;
    private RegistRecordAdapter mAdapter;

    public AssayRecordFragment() {

    }

    public static AssayRecordFragment newInstance(String tag) {
        AssayRecordFragment fragment = new AssayRecordFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mEmptyTextView.setText(R.string.no_assay_record);
        return contentView;
    ...
    private void getCheckReportAsync(ExT_Phr_CardBindRec cardInfo) {
        ApiCodeTemplate.loadAssayReportAsync(getBaseActivity(), TAG, null, cardInfo, getStartTime(), getEndTime(), new RequestCallback<List<G2110>>() {
            @Override
            public void onRequestSuccess(List<G2110> result) {
                CommonUtils.removeNull(result);
                if (result != null && !result.isEmpty()) {
                    mDataList.clear();
    ...
    public class CostsRecordViewHolder extends RecyclerView.ViewHolder {
        ...
        @Bind(R.id.txt_time)
        TextView txtTime;
        @Bind(R.id.location_colon)
        TextView locationColon;
        @Bind(R.id.ly_root)
        LinearLayout lyRoot;
    ...
    private class RegistRecordAdapter extends RecyclerView.Adapter<CostsRecordViewHolder> {
        ...
        public RegistRecordAdapter(Context context, List<G2110> records) {
        ...
        public void onBindViewHolder(CostsRecordViewHolder holder, int position) {
            ...
            // String dateString = TimeUtils.getTime(mRecords.get(position).getRepTime(), TimeUtils.DATE_FORMAT_MM);
            holder.txtAssayType.setText(mRecords.get(position).getRepName());
            holder.txtDoctorName.setText(mRecords.get(position).getRepDoctName());
            holder.txtLocation.setText(mRecords.get(position).getSendDeptName());
            holder.txtTime.setText(mRecords.get(position).getRepTime());
            holder.locationColon.setText("");
...
//                    if ("1".equals(mRecords.get(pos).getRepType())) {
//                        getBaseActivity().openActivity(AssayQueryDetailCommActivity.class, bundle);
//                    } else if ("2".equals(mRecords.get(pos).getRepType())) {
//                        getBaseActivity().openActivity(AssayQueryDetailMicoActivity.class, bundle);
//                    }
                    getBaseActivity().openActivity(AssayQueryDetailCommActivity.class, bundle);
                }
            });
